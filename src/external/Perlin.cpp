...
float Perlin::noise(float sample_x, float sample_y, float sample_z, float height, float scale, unsigned int offsetIndex)
{
	if(offsetIndex >= randOffsetPairVec.size()) {
		randOffsetPairVec.push_back(glm::vec3{float(rand() * 50 / (RAND_MAX/2)), float(rand() * 50 / (RAND_MAX/2)), float(rand() * 50 / (RAND_MAX/2))});
		offsetIndex = randOffsetPairVec.size() - 1;
	}

	sample_x += randOffsetPairVec[offsetIndex][0];
	sample_y += randOffsetPairVec[offsetIndex][1];
	sample_z += randOffsetPairVec[offsetIndex][2];
