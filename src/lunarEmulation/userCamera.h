...
public:
    ...
    UserCamera(glm::vec3 position = glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f), float yaw = YAW, float pitch = PITCH) : Front(glm::vec3(0.0f, 0.0f, -1.0f)), MovementSpeed(SPEED), Zoom(ZOOM)
    {
        Position = position;
        WorldUp = up;
        Yaw = yaw;
    ...
    void ProcessKeyboard(Camera_Movement direction, float deltaTime)
    {
        ...
        if (direction == FORWARD)
            ...
            ProcessMouseMovement(-TURNSPEED * deltaTime, 0);
        else if (direction == TURNRIGHT)
            ProcessMouseMovement(TURNSPEED * deltaTime, 0);
        else if (direction == TURNUP)
            ProcessMouseMovement(0, TURNSPEED * deltaTime);
